...
typeof verifyAndSyncDatabase !== 'undefined' || (async function verifyAndSyncDatabase() {
  try {
    await sequelize.authenticate();
    console.log('Conexión exitosa con la BD');
    await sequelize.sync(); // Crea tablas si no existen
    console.log('Sincronización de tablas completa');
  } catch (error) {
    console.error('Ocurrió un error con la conexión:', error);
  }
})();
...
app.post('/productos', async (req, res) => {
  try {
    ...
    const nuevoProducto = await Producto.create({ nombre, presentacion, descripcion, id_categoria, stock, precio, imagen });
